...
Une nouvelle couche ne sera ajoutée que lorsqu’un effet du Sprint 3 en aura réellement besoin.
...
Plusieurs instances d’un même `creatureId` pourront partager la même texture mise en cache.
...
Les futurs renforts apparaîtront dans le journal et le plateau. Une commande de développement peut être ajoutée temporairement pour les tests, mais elle ne constituera pas une règle de gameplay.
...
La sauvegarde reste actuellement en version 1. L’orientation de caméra n’est pas persistée et revient à `0°` après rechargement.

## Sauvegarde cible du Sprint 3

Si l’état évolue, la sauvegarde devra conserver :

- `creatureId` et `instanceId` ;
- compteur d’instances ;
- points de spawn et leur état utile ;
- instances apparues en cours de partie ;
- état de Brouhaha ;
- état des objets interactifs ;
- réactions résolues ou effets persistants ;
- version et migration.

Une reprise doit restaurer exactement les mêmes instances, positions et identifiants. Elle ne doit jamais régénérer les identifiants à partir de l’heure ou d’un nouvel aléa.
